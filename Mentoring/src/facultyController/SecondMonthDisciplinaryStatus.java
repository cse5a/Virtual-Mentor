...
/**
 * Servlet implementation class SecondMonthDisciplinaryStatus
 */
@WebServlet("/SecondMonthDisciplinaryStatus")
public class SecondMonthDisciplinaryStatus extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(session.getAttribute("USERAUTH") != null) {
			...
			List<FacultyBean> IdList = FacultyDao.getIdFromDisciplinaryStatusTable(StudentIds, SubjectCode, Department, Semester, Year);
			count=0;
			for(FacultyBean bean:IdList) {
				DatabaseIds[count] = bean.getId();
				count++;
			}
			Status = FacultyDao.assignSecondMonthDisciplinaryReport(DatabaseIds, StudentStatus, StudentRating);
			if(Status == true)
